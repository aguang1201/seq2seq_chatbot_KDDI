...
class Seq2SeqModel():
    ...
    def build_model(self):
        ...
        #上面的用法和下面直接使用sequence_mask的用法不是一样吗?
        # self.mask = tf.sequence_mask(self.decoder_targets_length, dtype=tf.float32, name='masks')
        ...
        with tf.variable_scope('encoder'):
            ...
            #使用tf.nn.bidirectional_dynamic_rnn构建单层双向的LSTM的RNN作为Encoder,考虑后期再上
            # encoder_f_cell = self._create_rnn_cell()
            # encoder_b_cell = self._create_rnn_cell()
        ...
        with tf.variable_scope('decoder'):
            ...
            if self.mode == 'train':
                ...
                decoder_input = tf.concat([tf.fill([self.batch_size, 1], self.word_to_idx[b"_GO"]), ending], 1)
                ...
                start_tokens = tf.ones([self.batch_size, ], tf.int32) * self.word_to_idx[b"_GO"]
                # start_tokens = tf.ones([self.batch_size, ], tf.int32) * self.word_to_idx["_GO"]
                # end_token = self.word_to_idx['<eos>']
                end_token = self.word_to_idx[b"_EOS"]
