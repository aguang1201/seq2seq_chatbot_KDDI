...
import sys
import numpy as np
from data_helpers import loadDataset_id_word
...
tf.app.flags.DEFINE_string('data_path', 'data', 'load data path')
FLAGS = tf.app.flags.FLAGS

word2id, id2word = loadDataset_id_word(FLAGS.data_path)
...
def predict_ids_to_seq(predict_ids, id2word, beam_szie):
    ...
    for single_predict in predict_ids:
        for i in range(beam_szie):
            ...
            print(" ".join(map(lambda predict_vocab: str(predict_vocab, encoding='UTF-8'), predict_seq)))
...
with tf.Session() as sess:
    ...
    while sentence:
        ...
        # batch = sentence_to_token_ids(sentence, word2id)
