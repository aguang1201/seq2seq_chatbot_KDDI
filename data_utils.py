...
def basic_tokenizer(sentence):
  ...
  words = []
  # sentence = convert(str(sentence, encoding="utf-8"), HF_ASCII)
...
def create_vocabulary(vocabulary_path, data_pathes, max_vocabulary_size, tokenizer=None, normalize_digits=True):
  ...
  if not gfile.Exists(vocabulary_path):
    print("Creating vocabulary %s from data %s" % (vocabulary_path, ','.join(data_pathes)))
    vocab = {}
    for data_path in data_pathes:
      with gfile.GFile(data_path, mode="rb") as f:
        ...
        for line in f:
          counter += 1
          if counter % 100000 == 0:
            print(f"{data_path}  processing line %d" % counter)
    ...
    vocab_list_str = list(map(lambda vocab: convert(str(vocab, encoding="utf-8"), HF_ASCII), vocab_list))
    # vocab_list_byte = list(map(lambda vocab: bytes(vocab, encoding="utf8"), vocab_list_str))
    vocab_list_str_sorted = sorted(set(vocab_list_str), key=vocab_list_str.index)
    vocab_list_all = _START_VOCAB + vocab_list_str_sorted
    # with gfile.GFile(vocabulary_path, mode="wb") as vocab_file:
    with gfile.GFile(vocabulary_path, mode="w") as vocab_file:
      # for w in vocab_list_byte:
      for w in vocab_list_all:
        vocab_file.write(w + "\n")
...
def prepare_my_data(data_dir, fr_vocabulary_size, tokenizer=None):
    ...
    data_pathes = [output_path, input_path]
    create_vocabulary(fr_vocab_path, data_pathes, fr_vocabulary_size, tokenizer)
    ...
    en_train_ids_path = input_path + (".ids%d.en" % fr_vocabulary_size)
    data_to_token_ids(output_path, fr_train_ids_path, fr_vocab_path, tokenizer)
    data_to_token_ids(input_path, en_train_ids_path, fr_vocab_path, tokenizer)
    return (en_train_ids_path, fr_train_ids_path, None, None, fr_vocab_path)


if __name__ == '__main__':
  prepare_my_data('data', 60000)
